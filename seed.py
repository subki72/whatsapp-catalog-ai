...
def seed_food_beverage_catalogs():
    ...
    if os.name == "nt" and database_url.startswith("sqlite:////app/"):
        raise RuntimeError(
            "DATABASE_URL masih mengarah ke path Docker. "
            "Untuk menjalankan seed secara lokal di Windows, gunakan "
            "'sqlite:///./catalog_db.sqlite' di file .env."
        )

    # Ensure the table exists even when seed.py is run without starting FastAPI first.
    ...
    try:
        if force_reseed:
            # Explicitly requested full reset before re-seeding.
        ...
        for data in dummies:
            existing_item = db.query(CatalogDB).filter(CatalogDB.user_id == data["user_id"]).first()

            if existing_item:
                existing_item.product_name = data["product_name"]
                existing_item.location = data["location"]
                existing_item.menus = data["menus"]
                existing_item.unique_selling_point = data["unique_selling_point"]
                updated_count += 1
                continue
        ...
        db.commit()
        print(
            f"Seed completed for database {database_url}. "
            f"Created: {created_count}, Updated: {updated_count}, Total input rows: {len(dummies)}"
        )
