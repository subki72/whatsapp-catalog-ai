...
- incoming WhatsApp messages are still unstructured
- an AI model is needed to extract catalog information
- the extracted result should be persisted in a lightweight local database
- the catalog should be browsable through a simple web interface
...
- Windows utility scripts for Docker-based testing and database reset
...
The current [seed.py](./seed.py) no longer skips execution just because the database already contains data.
